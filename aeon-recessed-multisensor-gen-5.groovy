metadata {
	...
	definition (name: "Aeon Multisensor 5th Gen", namespace: "jabbera", author: "Mike") {
...
def parse(String description)
{
	...
	if (cmd) {
		if( cmd.CMD == "8407" ) 
        { 
           result << secure(new physicalgraph.device.HubAction(zwave.wakeUpV1.wakeUpNoMoreInformation())) 
        }
		result << secure(createEvent(zwaveEvent(cmd)))
	}
...
def configure() {
	delayBetween([
		...
		secure(zwave.configurationV1.configurationSet(parameterNumber: 3, size: 2, scaledConfigurationValue: 15)),

		// send all data (temperature, humidity, illuminance & battery) periodically
		secure(zwave.configurationV1.configurationSet(parameterNumber: 101, size: 4, scaledConfigurationValue: 225)),

		// set data reporting period to 5 minutes
		secure(zwave.configurationV1.configurationSet(parameterNumber: 111, size: 4, scaledConfigurationValue: 300))
	])
}
